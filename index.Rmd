...
I created a subset of the dataset bball with variables, POSS.#, PLAY.ID, PASSES, POINT.RESULTS and called it reduce_data. When looking at the goodness of fit we can see that we get an average silhouette width of 0.62. This can be interpreted as a reasonable structure. Whe looking at the ggpair, the Poss.# and Play.ID have a very high correlation which should make sure since plays are correlated witht he number of possessions that are occurring. This also means that Texas most of the team is only having around 1 or 2 plays per possession. With the other variables in the data there is not much of a correlation such as there is not much of a correlation between how many passes in a possessions and the amount of points that is happening. 
...
princomp(bball_nums, cor=T) -> pca1
...
logistic_fit <- glm(`TRANSITION?` ~ PASSES, data = bball, family = 'binomial')
prob_reg <- predict (logistic_fit, type = 'response')

class_diag(prob_reg, bball$`TRANSITION?`, positive = "TRUE")

#Paint Touch and Game.#
logistic_fit <- glm(`TRANSITION?` ~ `GAME.#`, data = bball, family = 'binomial')
prob_reg <- predict (logistic_fit, type = 'response')

class_diag(prob_reg, bball$`TRANSITION?`, positive = "TRUE")

#Paint Touch and Point.result
logistic_fit <- glm(`TRANSITION?` ~ POINT.RESULT, data = bball, family = 'binomial')
prob_reg <- predict (logistic_fit, type = 'response')

class_diag(prob_reg, bball$`TRANSITION?`, positive = "TRUE")

#Paint Touch and Poss.#
logistic_fit <- glm(`TRANSITION?` ~ `POSS.#`, data = bball, family = 'binomial')
prob_reg <- predict (logistic_fit, type = 'response')

class_diag(prob_reg, bball$`TRANSITION?`, positive = "TRUE")

#Paint Touch and Play.ID
logistic_fit <- glm(`TRANSITION?` ~ PLAY.ID, data = bball, family = 'binomial')
prob_reg <- predict (logistic_fit, type = 'response')
...
for(i in 1:k){
  ...
  fit<-glm(as.factor(`TRANSITION?`)~PASSES,data=train,family="binomial")
...
knn_fit <- knn3(factor(`TRANSITION?`==1,levels=c("TRUE","FALSE")) ~ PASSES, data=bball, k=2)
y_hat_knn <- predict(knn_fit,bball)
head(y_hat_knn)


table(truth= factor(bball$PAINT.TOUCH==1, levels=c("TRUE","FALSE")),
      prediction= factor(y_hat_knn[,1]>.5, levels=c("TRUE","FALSE")))

class_diag(y_hat_knn[,1],bball$`TRANSITION?`, positive=1)
```
...
for(i in 1:k){
  ...
  truth<-test$`TRANSITION?` ## Truth labels for fold i
  ## Train model on training set (all but fold i)
  fit<-knn3(as.factor(`TRANSITION?`)~PASSES,data=train)
  ## Test model on test set (fold i) 
...
In the code, I first saved all of the passes values to a variable called data. Using python, I converted the r.data variable to a new_data in python and added up all of the passes finding that in the entire season Texas has 4471 passes in all and averaged 165.6 passes per game in the season. 
